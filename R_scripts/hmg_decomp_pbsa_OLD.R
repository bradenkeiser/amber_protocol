...
args=c('h:/HMG/tested_wt/liginfo_new/pbsa', 'WT','22.8503')
setwd(args[1])
pbsa_dat <- readLines('pbsa.dat') 
pbsa_line <- grep("DELTA TOTAL", pbsa_dat)
total_ddg <- args[3]
...
for (i in 1:length(dg_df$res)) {
  ...
  new_line <- paste(new_var[,1], new_val,sep = " ")
...
new_var <-  stringr::str_split(dg_df$res, "\\s+", simplify = FALSE)
my_test <-function(x) { 
...
dg_fig <- ggplot(fin, aes(x=new_reslist, y = fin_tot, fill = fin_res, label = fin_res)) +
  ...
  scale_x_discrete(breaks=new_reslist); dg_fig
...
dg_fig <- dg_fig + 
  annotate('text', x = xmax*.60, y = ymax*.6, hjust=0, vjust=1,
           label = paste('Complex PBSA DDG =',total_ddg, sep=' '), 
